...
use worklist_client_auth::{
    ...
    PrincipalSelection, UnlockMode, auth_response_to_credentials, clear_agent_credentials,
    clear_agent_seed, clear_credentials, credentials_path, load_agent_credentials,
    load_credentials, load_credentials_for_url, load_principal_credentials_for_url, login, logout,
...
use commands::agents::{AgentApproveArgs, AgentCommand};
    ...
    match command {
        ...
        Command::Agent { command } => match command {
            AgentCommand::Register { proposed_handle } => {
                commands::agents::cmd_agent_register(format, runtime.api_url(), proposed_handle)
                    .await
            }
            AgentCommand::Approve {
                ...
                commands::agents::cmd_agent_approve(AgentApproveArgs {
                    format,
                    runtime: &runtime,
                    deprecated_code: deprecated_code.as_deref(),
                    code_stdin,
                    code_file: code_file.as_deref(),
                    handle,
                    display_name,
                    password_stdin,
                })
            ...
            AgentCommand::List => commands::agents::cmd_agent_list(format, &runtime).await,
            AgentCommand::Revoke { selector } => {
                commands::agents::cmd_agent_revoke(format, &runtime, &selector).await
            }
        },
